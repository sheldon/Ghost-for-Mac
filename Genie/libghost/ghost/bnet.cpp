...
void  CBNET :: ProcessFileHashes( string EXEInfo, uint32_t EXEVersion, uint32_t EXEVersionHash )
{
	m_BNCSUtil->ProcessFileHashes( EXEInfo, EXEVersion, EXEVersionHash );
	...
	while( !m_Packets.empty( ) )
	{
		...
		if( Packet->GetPacketType( ) == BNET_HEADER_CONSTANT )
		{
			...
			case CBNETProtocol :: SID_AUTH_INFO:
				...
				{
					if( m_GHost->m_BNETHashCallback != NULL )
					{
						EventBNETHashRequestData data;
						data.formula = m_Protocol->GetValueStringFormulaString( );
						data.verString = m_Protocol->GetIX86VerFileNameString( );
						data.bnet = this;
						m_BNCSUtil->GenerateKeyHashes(m_GHost->m_TFT, m_CDKeyROC, m_CDKeyTFT,  m_Protocol->GetClientToken( ), m_Protocol->GetServerToken( ) );
						...
						m_GHost->m_BNETHashCallback(m_GHost->m_BNETHashCallbackObject, data);
					}
					else if( m_BNCSUtil->HELP_SID_AUTH_CHECK( m_GHost->m_TFT, m_GHost->m_Warcraft3Path, m_CDKeyROC, m_CDKeyTFT, m_Protocol->GetValueStringFormulaString( ), m_Protocol->GetIX86VerFileNameString( ), m_Protocol->GetClientToken( ), m_Protocol->GetServerToken( ) ) )
					{
