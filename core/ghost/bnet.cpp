		...
		if( Packet->GetPacketType( ) == BNET_HEADER_CONSTANT )
		{
			...
			case CBNETProtocol :: SID_AUTH_INFO:
				if( m_Protocol->RECEIVE_SID_AUTH_INFO( Packet->GetData( ) ) )
				{
					...
					{
						/*BYTEARRAY m_CalculatedExeVersion = m_BNCSUtil->GetEXEVersion();
						BYTEARRAY m_CalculatedExeVersionHash = m_BNCSUtil->GetEXEVersionHash();
						CONSOLE_Print( "[BNET: " + m_Server + "] calculated bnet_exeversion = " + UTIL_ToString( m_CalculatedExeVersion[0] ) + " " + UTIL_ToString( m_CalculatedExeVersion[1] ) + " " + UTIL_ToString( m_CalculatedExeVersion[2] ) + " " + UTIL_ToString( m_CalculatedExeVersion[3] ) );
						CONSOLE_Print( "[BNET: " + m_Server + "] calculated bnet_custom_exeversionhash = " + UTIL_ToString( m_CalculatedExeVersionHash[0] ) + " " + UTIL_ToString( m_CalculatedExeVersionHash[1] ) + " " + UTIL_ToString( m_CalculatedExeVersionHash[2] ) + " " + UTIL_ToString( m_CalculatedExeVersionHash[3] ) );
						CONSOLE_Print( "[BNET: " + m_Server + "] calculated exeinfo = " + m_BNCSUtil->GetEXEInfo( ) );*/
